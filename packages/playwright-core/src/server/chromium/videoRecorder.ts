...
export class VideoRecorder {
  ...
  private async _launch(options: types.PageScreencastOptions) {
    ...
    const threads = options.ffmpeg?.threads || 1;
    const qmin = options.ffmpeg?.qmin || 0;
    const qmax = options.ffmpeg?.qmax || 50;
    const crf = options.ffmpeg?.crf || 8;
    const bitrate = options.ffmpeg?.bitrate || '1M';
    const args = `-loglevel error -f image2pipe -avioflags direct -fpsprobesize 0 -probesize 32 -analyzeduration 0 -c:v mjpeg -i - -y -an -r ${fps} -c:v vp8 -qmin ${qmin} -qmax ${qmax} -crf ${crf} -deadline realtime -speed 8 -b:v ${bitrate} -threads ${threads} -vf pad=${w}:${h}:0:0:gray,crop=${w}:${h}:0:0`.split(' ');
